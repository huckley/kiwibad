...
bool computeHMACSignatureBinary(const uint8_t* message, size_t messageLen, uint8_t* outSig, size_t outSigLen) {
  if (outSigLen < SIG_LEN ) return false;  // Need at least 8 bytes for truncated signature
  ...
  // Copy the first x bytes of the HMAC to output buffer
...
bool verifyHMACSignatureBinary(const uint8_t* payload) {
  ...
  const uint8_t* receivedSig = payload + MSG_LEN;
  ...
  uint8_t expectedSig[SIG_LEN];
  if (!computeHMACSignatureBinary(message, MSG_LEN, expectedSig, SIG_LEN)) {
    return false;
  }

  bool match = (memcmp(receivedSig, expectedSig, SIG_LEN) == 0);
